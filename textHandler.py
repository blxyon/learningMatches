...
class TextHandlerApp():
    ...
    def createATxt(self):
        ...
        self.file_object.write("Note: '*'=reminder, '-'=activity\n")
    def formatLog(self):
    ...
    def writeAndDisplay(self,line):
        ...
        #by taking a substring we remove one of the "\n" because of print() functionality
        print(line[:(len(line)-1)])
    def getLastScore(self,sign):
        #gets the last score from the cached list list
        #we first find its line
        #This method shall only be used to cache the last score in the initialization stage!
        lineWithLastScore=0
    ...
    def getActNameAndDate(self,line):
        ...
        #this is a bit dumb such that the user must only type one activity in one line
        #if they put themselves in the txt more than one follwing "," the program will break
